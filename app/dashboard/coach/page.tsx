...
import { Brain, ChevronDown, ChevronUp, AlertTriangle, Target } from 'lucide-react'
...
export default function CoachCardsPage() {
  ...
  if (loading) return <div className="p-8 text-gray-400">Loading coach cards...</div>

  return (
    <div className="p-6 md:p-8 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-[#111]">AI Coach Cards</h1>
        <p className="text-gray-500 text-sm mt-0.5">Round-by-round breakdown — why you shot that score</p>
      </div>
      ...
      {cards.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
          ...
          <p className="text-gray-500 text-sm">No coach cards yet. Complete a round and tap "Get AI Analysis" in the app.</p>
        </div>
      ) : cards.map((card: any) => (
        <div key={card.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-4 overflow-hidden">
          ...
          >
            ...
            <div className="text-right shrink-0">
              <div className="text-[#C9A84C] font-black text-base">
            ...
            <div className="border-t border-gray-100">
              ...
              <div className="p-5 bg-red-50 flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm font-semibold text-[#111]">{card.biggest_leak}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                <div className="p-5">
                  ...
                  {card.club_misfiring && (
                    <div className="text-sm text-[#111] mb-1 flex items-center gap-1.5">
                      <span className="text-orange-400 font-bold">!</span> {card.club_misfiring}
                    </div>
                  )}
                  {card.miss_pattern && (
                    <div className="text-sm text-gray-500 mb-1">{card.miss_pattern}</div>
                  )}
                  {card.hot_club && (
                    <div className="text-sm text-[#22A06B] font-semibold mt-1">✓ Hot: {card.hot_club}</div>
                  )}
                </div>
                <div className="p-5">
                ...
                <div className="space-y-2.5">
                  {card.next_round_focus && (
                    <div className="flex gap-3 items-start">
                      ...
                      <div className="text-sm text-[#111] pt-0.5">{card.next_round_focus}</div>
                    </div>
                  )}
                  {card.practice_target && (
                    <div className="flex gap-3 items-start">
                      ...
                      <div className="text-sm text-[#111] pt-0.5">{card.practice_target}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              {card.ai_coach_text && (
                <div className="p-5 border-t border-gray-100">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">CLAUDE'S FULL ANALYSIS</div>
