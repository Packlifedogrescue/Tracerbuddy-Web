...
import { Watch, Activity, Zap, Wifi, WifiOff } from 'lucide-react'
...
export default function WatchPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-[13px] text-gray-400">Loading watch data…</div>
    </div>
  )
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {swings.slice(0, 20).map((s: any) => (
            <div key={s.id} className="flex items-center gap-4 px-5 py-3 border-b border-gray-50 last:border-0">
              ...
              <div className="flex-1">
                <div className="text-[13.5px] font-semibold text-[#111]">{s.club || 'Unknown club'}</div>
                <div className="text-[11.5px] text-gray-400 mt-0.5">
                  {format(new Date(s.recorded_at), 'MMM d, h:mm a')}
                </div>
              </div>
              <div className="text-right">
                <div className="font-black text-[#C9A84C] text-[14px]">
