...
import {
  Search, MapPin, Loader2, AlertCircle, Clock, Trophy, X,
  CloudSun, Phone, Mail, Globe, Info, ClipboardList,
} from 'lucide-react'
import { supabase } from '@/lib/supabase'
import WeatherWidget from '@/components/WeatherWidget'

// Free Leaflet satellite map (Esri tiles + our OSM GPS). Client-only.
const CourseMapFree = dynamic(() => import('@/components/CourseMapFree'), {
  ssr: false,
  loading: () => (
...
// GPS payload from /api/golf/raw-coordinates (OpenStreetMap).
interface GpsCoord { latitude: number; longitude: number }
interface GpsHole {
  hole: number | null
  par: number | null
  tee: GpsCoord | null
...
interface GpsData { source: string; centerSource?: 'course' | 'town' | null; center: GpsCoord | null; holes: GpsHole[]; bunkers?: GpsCoord[][]; water?: GpsCoord[][] }
        ...
        <div className="px-3 pt-3 pb-2 border-b border-[#F0EAE0]">
          ...
          {(rating || slope) && (
            <div className="flex gap-4 mt-2 text-[11px] text-gray-400">
              {rating ? <span>Rating <span className="font-bold text-[#111]">{String(rating)}</span></span> : null}
              {slope  ? <span>Slope <span className="font-bold text-[#111]">{String(slope)}</span></span> : null}
            </div>
          )}
        </div>
      )}
...
function CourseInfoPanel({ detail, lat, lng }: { detail: CourseDetail; lat: number | null; lng: number | null }) {
  ...
  if (detail.CourseType) rows.push({ label: 'Type', value: formatCourseType(detail.CourseType) })
  if (detail.NumHoles)   rows.push({ label: 'Holes', value: String(detail.NumHoles) })
  if (detail.Architect)  rows.push({ label: 'Architect', value: detail.Architect })
  if (detail.YearBuilt)  rows.push({ label: 'Est.', value: String(detail.YearBuilt) })
  if (detail.PriceRange) rows.push({ label: 'Green Fees', value: detail.PriceRange })
...
export default function CoursesPage() {
  ...
  async function search(overrideQuery?: string, overrideRegion?: string) {
    ...
    const ct = city.trim()
    if (!q && !rg && !ct) return
    setSearching(true); setResults([]); setSelected(null); setDetail(null); setGps(null); setError(''); setShowDropdown(true)
    try {
  ...
  async function searchVisited(name: string) {
    ...
    setQuery(q)
    setSearching(true); setResults([]); setShowDropdown(false); setSelected(null); setDetail(null); setGps(null); setError('')
    try {
  ...
  const lat     = parseNum(gps?.center?.latitude  ?? detail?.Latitude  ?? selected?.Latitude)
  const lng     = parseNum(gps?.center?.longitude ?? detail?.Longitude ?? selected?.Longitude)
  ...
  const TABS = [
    ...
    { id: 'weather',   label: 'Conditions', icon: CloudSun },
        ...
        <div className="flex items-center justify-between gap-4 mb-3">
          <div>
            ...
            <p className="text-[13px] text-gray-400 mt-0.5">
              Search 30,000+ courses worldwide for a satellite map, scorecard, and playing conditions
            </p>
          </div>
          {selected && (
            <button
            ...
            {visitedCourses.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-[13px] text-gray-400 max-w-[320px] mx-auto leading-relaxed">
                  30,000+ courses worldwide — satellite map, full scorecard, playing conditions, and more.
                </p>
              </div>
            )}
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11.5px] text-gray-400 mt-0.5">
                      ...
                      {detail?.Architect && <span>{detail.Architect}</span>}
                ...
                {loadingGps ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-[#F8F4EE] rounded-2xl">
                    <div className="text-center px-6">
                      ...
                      <p className="text-[13px] text-gray-400">No GPS map available for this course yet</p>
                      <p className="text-[11px] text-gray-300 mt-1">The scorecard and conditions still work</p>
                    </div>
                  </div>
                )}
              </div>
