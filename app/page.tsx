...
function StatNum({ to, suffix = '' }: { to: number; suffix?: string }) {
...
// Demo holes — Augusta National approximate coordinates
const DEMO_LAT  = 33.5030
const DEMO_LNG  = -82.0208
const DEMO_HOLES = [
  { HoleNo: 13, Par: 5, Yardage: 510, Handicap: 2,  TeeLatitude: 33.5005, TeeLongitude: -82.0230, GreenLatitude: 33.4997, GreenLongitude: -82.0193 },
  { HoleNo: 16, Par: 3, Yardage: 170, Handicap: 14, TeeLatitude: 33.5020, TeeLongitude: -82.0183, GreenLatitude: 33.5012, GreenLongitude: -82.0173 },
  { HoleNo: 18, Par: 4, Yardage: 465, Handicap: 6,  TeeLatitude: 33.5045, TeeLongitude: -82.0192, GreenLatitude: 33.5057, GreenLongitude: -82.0205 },
]
...
export default function HomePage() {
  ...
  const [mapHole, setMapHole] = useState<number | undefined>(undefined)
  ...
  return (
    <div className="bg-[#F5EFE0] text-[#1A1A1A] min-h-screen font-sans grain">

      {/* ════════════════════════ NAV ════════════════════════ */}
      <nav className="sticky top-0 z-50 bg-[#F5EFE0]/95 backdrop-blur-sm border-b border-black/[0.04]">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '90vh' }}>
        {/* Course photo fades in from the right */}
        <div className="absolute inset-0">
          <Image src="/images/hero-course.jpg" alt="" fill className="object-cover object-center" priority />
          <div className="absolute inset-0" style={{
            background: 'linear-gradient(108deg, #F5EFE0 0%, #F5EFE0 36%, rgba(245,239,224,0.92) 50%, rgba(245,239,224,0.55) 66%, rgba(245,239,224,0.1) 100%)',
          }} />
        </div>

        <div className="relative z-10 px-6 md:px-12 pt-16 md:pt-28 pb-20 max-w-[1400px] mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center" style={{ minHeight: 'calc(90vh - 160px)' }}>

            <Reveal className="relative z-10">
              <h1 className="font-serif text-[52px] md:text-[82px] font-medium leading-[1.0] tracking-[-0.03em] mb-7">
                Track Every Shot.<br />
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                ...
                <a href="#features" className="premium-btn-ghost">See How It Works</a>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <div className="inline-flex items-center gap-2.5 bg-[#1A1A1A]/[0.06] border border-black/[0.08] px-4 py-2.5 rounded-xl">
                  ...
                  <div>
                    <div className="text-[9px] leading-none text-[#999] mb-0.5">Now in Beta</div>
                    <div className="text-[14px] font-semibold leading-none text-[#555]">Join TestFlight</div>
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-5 text-[12px] text-[#888]">
                {['1 round free', 'No credit card', 'Cancel anytime'].map(t => (
            ...
            <Reveal delay={200} className="relative flex items-center justify-center">
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="w-[500px] h-[500px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(223,153,5,0.2) 0%, rgba(223,153,5,0.06) 40%, transparent 70%)' }} />
              </div>
              <Image
                src="/images/hero-phone.png"
                alt="TracerBuddy app on iPhone"
                width={1024} height={1536}
                className="relative max-h-[740px] max-w-full w-auto object-contain mix-blend-multiply"
                style={{ filter: 'drop-shadow(0 60px 120px rgba(26,26,26,0.28)) drop-shadow(0 20px 40px rgba(26,26,26,0.18))' }}
                priority
              />
            </Reveal>
          </div>
      ...
      <section className="px-6 md:px-12 py-12 max-w-[1400px] mx-auto">
        <Reveal>
          <div className="border-y border-black/[0.06] py-8">
            <div className="grid grid-cols-3 gap-8 text-center">
              <div>
                <div className="font-serif text-[32px] md:text-[44px] font-medium text-[#1A1A1A] leading-none">
                  <StatNum to={42000} suffix="+" />
                </div>
                <div className="text-[12px] font-semibold text-[#1A1A1A] mt-2 tracking-wide">Courses</div>
                <div className="text-[11px] text-[#888] mt-0.5">in 100+ countries</div>
              </div>
              <div>
                <div className="font-serif text-[32px] md:text-[44px] font-medium text-[#1A1A1A] leading-none">
                  <StatNum to={18} />
                </div>
                <div className="text-[12px] font-semibold text-[#1A1A1A] mt-2 tracking-wide">Metrics Per Round</div>
                <div className="text-[11px] text-[#888] mt-0.5">GIR, putts, SG & more</div>
              </div>
              <div>
                <div className="font-serif text-[32px] md:text-[44px] font-medium text-[#1A1A1A] leading-none">
                  <StatNum to={6} />
                </div>
                <div className="text-[12px] font-semibold text-[#1A1A1A] mt-2 tracking-wide">Game Formats</div>
                <div className="text-[11px] text-[#888] mt-0.5">stroke, match, skins & more</div>
              </div>
            </div>
          </div>
        </Reveal>
      ...
      {/* ════════════════════════ LIVE 3D MAP ════════════════════════ */}
      <section className="px-6 md:px-12 pb-20 max-w-[1400px] mx-auto">
        <Reveal>
          <div className="text-center mb-8">
            <div className="text-[11px] font-bold text-[#DF9905] tracking-[0.25em] mb-4">INTERACTIVE COURSE MAP</div>
            <h2 className="font-serif text-3xl md:text-[48px] font-medium tracking-[-0.02em] leading-[1.1]">
              See every hole in 3D.<br className="hidden md:block" />Before you ever tee off.
            </h2>
            <p className="text-[14.5px] text-[#666] mt-5 max-w-[480px] mx-auto leading-[1.65]">
              Real satellite imagery, terrain elevation, and animated hole paths — for 42,000+ courses worldwide.
            </p>
          </div>
        </Reveal>
        <Reveal delay={100}>
          <div
            className="relative rounded-3xl overflow-hidden border border-black/[0.06]"
            style={{ height: 600, boxShadow: '0 2px 0 rgba(255,255,255,0.8) inset, 0 40px 80px rgba(0,0,0,0.12), 0 8px 24px rgba(0,0,0,0.06)' }}
          >
            <CourseMapbox
              lat={DEMO_LAT}
              lng={DEMO_LNG}
              ...
              selectedHole={mapHole}
              onHoleClick={setMapHole}
            />
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 pointer-events-none">
              <div className="bg-black/65 backdrop-blur-md border border-white/[0.08] px-5 py-2 rounded-full text-white text-[11px] font-semibold tracking-wide whitespace-nowrap">
                Click any hole · Use &quot;Fly Tour&quot; for a cinematic walkthrough
              </div>
            </div>
          </div>
        </Reveal>
      ...
      <section className="px-6 md:px-12 pb-12 max-w-[1400px] mx-auto">
        <Reveal>
          <div
            className="bg-gradient-to-br from-[#EFE7D4] to-[#E5DBC2] rounded-3xl px-8 md:px-16 py-14 text-center relative overflow-hidden"
            style={{ boxShadow: '0 2px 0 rgba(255,255,255,0.8) inset, 0 20px 60px rgba(0,0,0,0.07), 0 4px 16px rgba(0,0,0,0.04)' }}
          >
            ...
            <div className="relative z-10 max-w-[540px] mx-auto">
              ...
              {waitlistDone ? (
                ...
                <form onSubmit={handleWaitlist} className="flex flex-col sm:flex-row gap-3 max-w-[420px] mx-auto">
                  <input type="email" value={waitlistEmail} onChange={e => setWaitlistEmail(e.target.value)} placeholder="you@email.com" required
          ...
          {[
            ...
            <Reveal key={step.num} delay={i * 100}>
              <div className="text-center relative">
                <div
                  className="w-14 h-14 rounded-full bg-[#1A1A1A] text-[#F5EFE0] flex items-center justify-center font-serif font-medium text-[18px] mx-auto mb-6 relative z-10"
      ...
      <section id="features" className="px-6 md:px-12 pb-20 max-w-[1400px] mx-auto">
        ...
        <Reveal>
          <div className="flex flex-wrap justify-center gap-2.5 mb-12">
            {['Digital Scorecards','Shot Tracking','Satellite Maps','Apple Watch','Strokes Gained','Club Distances','AI Coach Cards','Putting Stats','Match Play','Stableford','Practice Mode','Buddy Battles'].map(tag => (
              <span key={tag} className="text-[12.5px] text-[#555] bg-white/70 border border-black/[0.06] px-3.5 py-1.5 rounded-full">{tag}</span>
            ))}
          </div>
        </Reveal>

        <div
          className="grid md:grid-cols-3 gap-px bg-black/[0.06] rounded-2xl overflow-hidden border border-black/[0.06]"
          style={{ boxShadow: '0 4px 0 rgba(255,255,255,0.7) inset, 0 24px 60px rgba(0,0,0,0.07), 0 4px 16px rgba(0,0,0,0.04)' }}
        >
          {([
            {
              num: '01', label: 'SCORING & ROUNDS', title: 'Digital Scorecards',
              ...
              mockup: (
                <div className="mt-4 bg-[#1A1A1A] rounded-xl p-3">
                  <div className="text-[8px] text-[#555] mb-2 font-bold tracking-widest">TODAY&apos;S ROUND</div>
                  <div className="grid grid-cols-5 gap-1">
                    {[{h:1,s:4,p:4},{h:2,s:3,p:3},{h:3,s:5,p:4},{h:4,s:4,p:4},{h:5,s:6,p:5}].map(r => (
                      <div key={r.h} className="text-center">
                        <div className="text-[7px] text-[#444] mb-0.5">{r.h}</div>
                        <div className={`text-[12px] font-bold ${r.s < r.p ? 'text-[#DF9905]' : r.s > r.p ? 'text-[#ef4444]' : 'text-white'}`}>{r.s}</div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {
              num: '02', label: 'SHOT TRACKING', title: 'Precision Shot Tracking',
              ...
              mockup: (
                <div className="mt-4 bg-[#0d2010] rounded-xl overflow-hidden relative" style={{ height: 84 }}>
                  <svg viewBox="0 0 200 84" className="w-full h-full">
                    <rect width="200" height="84" fill="#0d2010"/>
                    <path d="M100,10 Q110,25 108,42 Q106,58 100,74" stroke="rgba(34,197,94,0.45)" strokeWidth="16" fill="none" strokeLinecap="round"/>
                    <path d="M100,72 Q104,48 101,18" stroke="#DF9905" strokeWidth="1.5" fill="none" strokeDasharray="3,2" opacity="0.9"/>
                    <circle cx="100" cy="72" r="3" fill="#DF9905"/>
                    <circle cx="101" cy="18" r="2.5" fill="white"/>
                    <text x="112" y="46" fill="white" fontSize="9" fontWeight="bold" fontFamily="system-ui">187y</text>
                  </svg>
                </div>
              ),
            },
            {
              num: '03', label: 'COURSE MAPS', title: 'Satellite Course Maps',
              ...
              mockup: (
                <div className="mt-4 rounded-xl overflow-hidden relative" style={{ height: 84 }}>
                  <svg viewBox="0 0 200 84" className="w-full h-full">
                    <rect width="200" height="84" fill="#1c4a2c"/>
                    <path d="M100,76 Q112,52 108,30 Q104,14 100,6" stroke="rgba(74,222,128,0.55)" strokeWidth="18" fill="none" strokeLinecap="round"/>
                    <path d="M100,76 Q108,50 104,18" stroke="white" strokeWidth="2" fill="none" strokeDasharray="4,3" opacity="0.8"/>
                    <circle cx="100" cy="6" r="3.5" fill="#ef4444"/>
                    <line x1="100" y1="6" x2="100" y2="14" stroke="white" strokeWidth="1.5"/>
                    <rect x="86" y="70" width="28" height="10" rx="5" fill="rgba(201,168,76,0.9)"/>
                    <text x="100" y="77.5" fill="white" fontSize="6" fontWeight="800" textAnchor="middle" fontFamily="system-ui">Hole 7</text>
                    <text x="116" y="43" fill="#DF9905" fontSize="9" fontWeight="bold" fontFamily="system-ui">164y</text>
                  </svg>
                </div>
              ),
            },
            {
              num: '04', label: 'PERFORMANCE INSIGHTS', title: 'Stats That Move the Needle',
              ...
              mockup: (
                <div className="mt-4 grid grid-cols-3 gap-1.5">
                  {[{l:'GIR',v:'64%',c:'#22c55e'},{l:'FIR',v:'52%',c:'#DF9905'},{l:'Putts',v:'1.84',c:'#60a5fa'}].map(s => (
                    <div key={s.l} className="bg-[#1A1A1A] rounded-lg p-2 text-center">
                      <div className="text-[8px] text-[#555] mb-1 font-semibold">{s.l}</div>
            ...
            {
              num: '05', label: 'AI COACH', title: 'AI Coach Cards',
              ...
              mockup: (
                <div className="mt-4 bg-[#FFFBF0] border border-[#EDE8DC] rounded-xl p-3">
                  <div className="text-[8px] font-bold text-[#DF9905] tracking-widest mb-1.5">AI COACH · TODAY</div>
                  <div className="text-[10px] text-[#333] leading-relaxed">Your wedge game cost 2.3 strokes. Focus on 80–100y approaches this week.</div>
                  <div className="mt-2.5 h-1 rounded-full bg-black/[0.06]">
                    <div className="h-1 rounded-full bg-[#DF9905]" style={{ width: '68%' }}/>
                  </div>
                </div>
              ),
            },
            {
              num: '06', label: 'SOCIAL & COMPETITION', title: 'Buddy Battles',
              ...
              mockup: (
                <div className="mt-4 bg-[#1A1A1A] rounded-xl p-3">
                  <div className="text-[8px] text-[#444] mb-2 font-bold tracking-widest">LEADERBOARD</div>
                  {[{name:'You',score:'-2',gold:true},{name:'Mike',score:'E',gold:false},{name:'Tyler',score:'+3',gold:false}].map(p => (
                    <div key={p.name} className="flex items-center justify-between py-0.5">
                      <span className="text-[9px] text-white">{p.name}</span>
                      <span className={`text-[9px] font-bold ${p.gold ? 'text-[#DF9905]' : 'text-[#666]'}`}>{p.score}</span>
                    </div>
                  ))}
                </div>
              ),
            ...
            <Reveal key={cat.num} delay={i * 60}>
              <div className="group h-full bg-[#F5EFE0] hover:bg-white transition-colors duration-300 p-7 md:p-8">
                ...
                <p className="text-[13px] text-[#777] leading-[1.6]">{cat.desc}</p>
                {cat.mockup}
        ...
        <Reveal delay={100}>
          <div
            className="mt-5 bg-[#1A1A1A] text-[#F5EFE0] rounded-2xl p-7 md:p-10 grid md:grid-cols-2 gap-8 items-center relative overflow-hidden"
            style={{ boxShadow: '0 40px 80px rgba(0,0,0,0.2), 0 8px 24px rgba(0,0,0,0.12)' }}
          >
      ...
      <section id="watch" className="px-6 md:px-12 pb-8 max-w-[1400px] mx-auto">
        <Reveal>
          <div
            className="bg-gradient-to-br from-[#EFE7D4] via-[#EFE7D4] to-[#E5DBC2] rounded-3xl px-8 md:px-16 pt-14 pb-0 overflow-hidden relative"
            style={{ boxShadow: '0 4px 0 rgba(255,255,255,0.8) inset, 0 24px 60px rgba(0,0,0,0.08), 0 4px 16px rgba(0,0,0,0.04)' }}
          >
                  ...
                  {[
                    ...
                    <div key={item} className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full bg-[#DF9905] flex items-center justify-center flex-shrink-0"
                        style={{ boxShadow: '0 4px 12px rgba(223,153,5,0.4)' }}>
      ...
      <section className="px-4 md:px-12 pb-8 max-w-[1400px] mx-auto">
        <Reveal>
          <div
            className="bg-[#1A1A1A] text-[#F5EFE0] rounded-3xl overflow-hidden relative"
            style={{ boxShadow: '0 40px 80px rgba(0,0,0,0.18), 0 8px 24px rgba(0,0,0,0.1)' }}
          >
              ...
              <div className="md:col-span-8 grid grid-cols-3 gap-2.5 md:gap-4 relative z-10">
                {[
                  ...
                  <div key={m.label} className="text-center bg-white/[0.04] backdrop-blur border border-white/[0.06] rounded-2xl p-3 md:p-5"
                    style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
                    ...
                    <svg width="100%" height="16" viewBox="0 0 100 20" className="mt-2 md:mt-3">
                      <path d="M0,12 Q15,4 30,10 T60,6 T90,8 T100,5" stroke="#DF9905" strokeWidth="1.2" fill="none" strokeLinecap="round" opacity="0.7"/>
                    </svg>
                  </div>
                ))}
              </div>
      ...
      <section id="pricing" className="px-6 md:px-12 py-20 max-w-[1400px] mx-auto">
        <Reveal>
          <div className="text-center mb-12">
            ...
            <h2 className="font-serif text-3xl md:text-[48px] font-medium tracking-[-0.02em] leading-[1.1] mb-4">Invest in your game.</h2>
            <p className="text-[14.5px] text-[#666] max-w-[460px] mx-auto leading-[1.6]">Full access from day one. No tiers, no add-ons, no hidden fees.</p>
          </div>
        </Reveal>
        <div className="grid md:grid-cols-2 gap-5 max-w-3xl mx-auto">
          <Reveal delay={50}>
            <div className="h-full bg-white border border-black/[0.05] rounded-2xl p-8"
              style={{ boxShadow: '0 4px 0 rgba(255,255,255,0.9) inset, 0 20px 50px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)' }}>
          ...
          <Reveal delay={150}>
            <div className="h-full bg-[#1A1A1A] text-[#F5EFE0] rounded-2xl p-8 relative border border-[#DF9905]/40"
              style={{ boxShadow: '0 0 0 1px rgba(223,153,5,0.1), 0 40px 80px rgba(223,153,5,0.18), 0 8px 24px rgba(0,0,0,0.2)' }}>
              <div className="absolute -top-3 right-6 bg-[#DF9905] text-white text-[10px] font-bold px-3 py-1 rounded-full tracking-[0.15em]"
                style={{ boxShadow: '0 4px 12px rgba(223,153,5,0.4)' }}>SAVE $40</div>
      ...
      <section className="px-6 md:px-12 py-14 max-w-[1400px] mx-auto">
        <Reveal>
          <div className="relative rounded-3xl overflow-hidden bg-[#1A1A1A]"
            style={{ boxShadow: '0 40px 80px rgba(0,0,0,0.2), 0 8px 24px rgba(0,0,0,0.1)' }}>
