...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    safe(db.from('club_sessions').select('*', { count: 'exact', head: true })),
    safe(db.from('putt_data').select('*', { count: 'exact', head: true })),
    safe(db.from('community_posts').select('*', { count: 'exact', head: true })),
    safe(db.from('rounds').select('*', { count: 'exact', head: true }).in('round_mode', ['Match Play', 'Stableford', 'Skins', 'Best Ball', 'Tournament'])),
