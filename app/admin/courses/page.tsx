        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && courses.length === 0 && (
              <tr><td colSpan={6} className="text-center py-12 text-gray-600 text-sm">No courses found</td></tr>
            )}
            {!loading && courses.map(c => (
              <tr key={c.id} className="border-t border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 text-center text-gray-300">{c.par ?? '—'}</td>
                <td className="px-4 py-3 text-center text-gray-300">{c.course_rating ?? '—'}</td>
                <td className="px-4 py-3 text-center text-gray-300">{c.slope_rating ?? '—'}</td>
